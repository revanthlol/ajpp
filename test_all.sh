...
(cd unit2_linux/prog05_javabean && ./run.sh)

echo -e "\n[2/6] Program 6 (XML Root & Child Nodes)..."
(cd unit2_linux/prog06_xml_nodes && ./run.sh)

echo -e "\n[3/6] Program 7 (DOM Tree Structure)..."
(cd unit2_linux/prog07_dom_tree && ./run.sh)

echo -e "\n[4/6] Compiling Servlets (Prog 8, 9, 10)..."
(cd unit2_linux/prog08_servlet_hello && ./compile.sh)
(cd unit2_linux/prog09_servlet_calculator && ./compile.sh)
(cd unit2_linux/prog10_servlet_login && ./compile.sh)

echo -e "\n[5/6] Running JDBC Create Table & Insert Data..."
(cd unit2_linux/prog13_jdbc_create_table && ./run.sh)
(cd unit2_linux/prog14_jdbc_insert && ./run.sh)

echo -e "\n[6/6] Running JDBC Select Data..."
(cd unit2_linux/prog15_jdbc_select && ./run.sh)

# Clean temporary build files
find unit1 unit2_linux -name "*.class" -o -name "*.db" -delete 2>/dev/null
